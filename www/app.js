const titles={overview:'Battery Health',battery:'Batterij',trips:'Ritten',controls:'Bediening',settings:'Instellingen'};
...
document.querySelector('#locate').onclick=async()=>{showToast('Nieuwste voertuiglocatie ophalen…');const ok=await refreshTelemetry(true);if(ok&&!document.querySelector('#locationName').textContent.includes(','))showToast('Nog geen locatie ontvangen. De Tesla stuurt deze zodra hij online is.')};
async function refreshOnOpen(){await refreshTelemetry(false)}
document.addEventListener('DOMContentLoaded',()=>{document.body.classList.add('overview-open');const bar=window.Capacitor?.Plugins?.StatusBar;if(bar){bar.setOverlaysWebView({overlay:true});bar.setStyle({style:'LIGHT'})}refreshOnOpen();setInterval(refreshTelemetry,60000)});
